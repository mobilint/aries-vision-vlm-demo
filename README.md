...
- Current sample assets include:
  - `backend_vision/assets/mxq/yolo26s-weapon_uint8_input.mxq`
...
The downloader uses `uv` to create a local `.hf_venv`, installs `huggingface-hub`, and runs `hf buckets sync` from the default bucket:
...
- `HF_ASSET_BUCKET_ID`: Hugging Face bucket ID, default `mobilint/aries-weapon-detection-demo-assets`
