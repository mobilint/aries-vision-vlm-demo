...
std::map<std::string, VisionCoreAllocation> LoadVisionCoreAllocation(
    ...
    std::map<std::string, VisionCoreAllocation> result;
    for (auto it = vision_node.begin(); it != vision_node.end(); ++it) {
        ...
        VisionCoreAllocation alloc;
        // dev_no is per-category (each YOLO model binds to exactly one card).
        // Each list entry may carry its own dev_no for readability, but they
        // must all agree; the first non-null value wins, subsequent mismatches
        // raise. Omitting the field on every entry leaves dev_no unset and
        // the caller keeps ModelSetting's default.
        for (int i = 0; i < static_cast<int>(cores_node.size()); ++i) {
            CoreId c;
            c.cluster = cores_node[i]["cluster"].as<int>();
            c.core = cores_node[i]["core"].as<int>();
            alloc.cores.push_back(c);
            if (cores_node[i]["dev_no"]) {
                int entry_dev_no = cores_node[i]["dev_no"].as<int>();
                if (!alloc.dev_no.has_value()) {
                    alloc.dev_no = entry_dev_no;
                } else if (entry_dev_no != *alloc.dev_no) {
                    throw std::runtime_error(
                        "core_allocation.yaml vision." + category +
                        " has inconsistent dev_no across entries");
                }
            }
        }
        result.emplace(std::move(category), std::move(alloc));
    }
    return result;
}
