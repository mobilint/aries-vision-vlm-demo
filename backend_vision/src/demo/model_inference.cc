...
cv::Mat Model::inferenceYolo11(cv::Mat frame, cv::Size size, int stream_id) {
    ...
    const bool only_falling_detections = !mLabels.empty() && mLabels[0] == "falling";
    std::vector<DetectionBox> latest_detections;
    for (int i = 0; i < (int)boxes.size(); i++) {
        if (only_falling_detections && labels[i] != 0) continue;
        ...
        drawDetectionOverlay(scratch.result_frame, roi, labels[i], ema_scores[i]);
        latest_detections.push_back(
            {roi, ema_scores[i], labels[i], getLabelName(labels[i])});
    }
...
cv::Mat Model::inferenceYolo26(cv::Mat frame, cv::Size size, int stream_id) {
    ...
    const int nc = std::max(1, (int)mLabels.size());
    const float conf_thres = mPipelineConfig.conf_threshold;
