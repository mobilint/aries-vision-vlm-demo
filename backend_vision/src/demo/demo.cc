...
void Demo::initModels(std::string path) {
    ...
    const std::string allocation_path = ResolveCoreAllocationPath("");
    std::map<std::string, VisionCoreAllocation> allocation =
        LoadVisionCoreAllocation(allocation_path);

    static const mobilint::Cluster kClusters[] = {
        mobilint::Cluster::Cluster0,
        mobilint::Cluster::Cluster1,
    };
    static const mobilint::Core kCores[] = {
        mobilint::Core::Core0,
        mobilint::Core::Core1,
        mobilint::Core::Core2,
        mobilint::Core::Core3,
    ...
    for (size_t i = 0; i < mModelSetting.size(); i++) {
        ModelSetting& ms = mModelSetting[i];
        if (ms.category.empty()) {
            throw std::runtime_error(
                "ModelSetting entry " + std::to_string(i) +
                " is missing required 'category' field");
        }
        auto it = allocation.find(ms.category);
        if (it == allocation.end()) {
            throw std::runtime_error(
                "core_allocation.yaml (" + allocation_path +
                ") is missing vision category '" + ms.category + "'");
        }
        const VisionCoreAllocation& alloc = it->second;
        const std::vector<CoreId>& cores = alloc.cores;
        if (cores.empty()) {
            throw std::runtime_error(
                "core_allocation.yaml (" + allocation_path + ") vision." +
                ms.category + " has no cores assigned");
        }

        // core_allocation.yaml is the single source of truth for dev_no when
        // it names one; ModelSetting.dev_no (a legacy default) only survives
        // when the allocation entry omits dev_no entirely.
        if (alloc.dev_no.has_value()) {
            ms.dev_no = *alloc.dev_no;
        }

        ms.core_id.clear();
        for (const CoreId& c : cores) {
            if (c.cluster < 0 || c.cluster >= static_cast<int>(
                                                  sizeof(kClusters) / sizeof(kClusters[0]))) {
                throw std::runtime_error(
                    "core_allocation.yaml vision." + ms.category +
                    " has out-of-range cluster " + std::to_string(c.cluster));
            }
            if (c.core < 0 ||
                c.core >= static_cast<int>(sizeof(kCores) / sizeof(kCores[0]))) {
                throw std::runtime_error(
                    "core_allocation.yaml vision." + ms.category +
                    " has out-of-range core " + std::to_string(c.core));
            }
            ms.core_id.push_back({kClusters[c.cluster], kCores[c.core]});
        }
        if (!ms.is_num_core) {
            ms.num_core = static_cast<int>(ms.core_id.size());
        }
    }
...
std::string Demo::getDetectionSnapshotJson() const {
    ...
    for (const auto& snapshot : snapshots) {
        ...
        std::string category;
        if (snapshot.model_index >= 0 &&
            (size_t)snapshot.model_index < mModelSetting.size()) {
            category = mModelSetting[snapshot.model_index].category;
        }
...
std::string Demo::getLayoutMetadataJson() const {
    ...
    for (size_t wi = 0; wi < mLayoutSetting.worker_layout.size(); wi++) {
        ...
        std::string category;
        if (worker.model_index >= 0 &&
            (size_t)worker.model_index < mModelSetting.size()) {
            category = mModelSetting[worker.model_index].category;
        }
