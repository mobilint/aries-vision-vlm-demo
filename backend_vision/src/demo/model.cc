...
Model::Model(const ModelSetting& model_setting, mobilint::Accelerator& acc) {
    ...
    mPipelineConfig = model_setting.pipeline_config;
    mLabels = mPipelineConfig.labels.empty() ? model_setting.labels : mPipelineConfig.labels;
    mScoreEmaDisplayThres = mPipelineConfig.display_confidence_threshold;
...
void Model::initYolo11() {
    float conf_thres = mPipelineConfig.conf_threshold;
    float iou_thres = mPipelineConfig.iou_threshold;

    int w = mModel->getInputBufferInfo()[0].original_width;
    int h = mModel->getInputBufferInfo()[0].original_height;
    int nc = std::max(1, (int)mLabels.size());
...
std::string Model::getLabelName(int label) const {
    if (label >= 0 && (size_t)label < mLabels.size() && !mLabels[label].empty()) {
...
std::string Model::buildDetectionText(int label, float confidence) const {
    std::string text;
    if (mPipelineConfig.draw_label_text) {
        text = getLabelName(label);
    }
    if (mPipelineConfig.draw_score_text) {
        char conf_text[32];
        std::snprintf(conf_text, sizeof(conf_text), "%.2f", confidence);
        if (!text.empty()) {
            text += " ";
        }
        text += conf_text;
...
void Model::drawDetectionOverlay(cv::Mat& image, const cv::Rect& roi, int label,
                                 float confidence) const {
    cv::Scalar clr = (label == 0) ? cv::Scalar(255, 0, 255) : cv::Scalar(0, 255, 255);
    if (mPipelineConfig.draw_detection_border) {
        cv::rectangle(image, roi, clr, mPipelineConfig.bbox_thickness);
    }

    std::string text = buildDetectionText(label, confidence);
    if (text.empty()) {
        return;
    }
    int text_y = std::max(14, roi.y - 6);
    cv::putText(image, text, cv::Point(roi.x, text_y), cv::FONT_HERSHEY_SIMPLEX,
                0.5, clr, 1, cv::LINE_AA);
}
