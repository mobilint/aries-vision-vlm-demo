...
// Per-category vision core assignment loaded from core_allocation.yaml.
// `dev_no` selects the Aries card (mbltml/qbruntime device index); if the
// yaml omits it, this is std::nullopt and the caller keeps ModelSetting's
// existing dev_no. `cores` are the (cluster, core) tuples within that card
// that the model pins to.
struct VisionCoreAllocation {
    std::optional<int> dev_no;
...
struct PipelineConfig {
    ...
    bool draw_detection_border = true;
    std::vector<std::string> allowed_label_names;
};
