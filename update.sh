...
echo "Downloading vision assets (mxq/mp4) from Hugging Face buckets..."
sudo UV_USER_HOME="$USER_HOME" bash ./download.sh

echo "Skipping backend_vision native host build."
echo "backend_vision is built inside Docker to keep build-time and runtime OpenCV versions consistent."
...
echo "Registering login-time autostart for the kiosk demo..."
AUTOSTART_DIR="$USER_HOME/.config/autostart"
sudo mkdir -p "$AUTOSTART_DIR" || {
  echo "Failed to create autostart directory at $AUTOSTART_DIR."
  exit 1
}
sudo cp vision-vlm-demo.desktop "$AUTOSTART_DIR/"
sudo chown -R "$RUN_AS_USER:$RUN_AS_USER" "$AUTOSTART_DIR"
echo "Autostart registered: $AUTOSTART_DIR/vision-vlm-demo.desktop"
