...
const VLM_WARNING_KEYWORD_REGEX = /\b(weapon|gun|knife|rifle|pistol|firearm|흉기|총|칼)\b/gi;

function emphasizeWarningKeywords(text: string): string {
  return text.replace(VLM_WARNING_KEYWORD_REGEX, "**$1**");
}

function isWarningKeywordNode(children: React.ReactNode): boolean {
  const text = Array.isArray(children) ? children.join("") : String(children ?? "");
  return /^(weapon|gun|knife|rifle|pistol|firearm|흉기|총|칼)$/i.test(text.trim());
}
      ...
      {thought_and_answer ?
        ...
        {thought &&
          ...
          >
            ...
            >
              {highlightedThought + (isAnswering && !!answer == false ? " ..." : "")}
            </ReactMarkdown>
          </Grid2>
        }{real_answer &&
          <Grid2
