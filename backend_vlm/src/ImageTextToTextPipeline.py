...
class PresencePenaltyLogitsProcessor(LogitsProcessor):
    ...
    touches the EOS token (which has not been generated yet), so it
    discourages repetition without suppressing termination. Enabled by a
    "presence_penalty" key in the generation config JSON; recommended by the
    Qwen3-VL model cards (1.5) against repetition in quantized models."""
...
class ImageTextToTextPipeline:
    ...
    CATEGORY_DEFAULT_MODEL: Dict[str, str] = {
        "weapon": "Qwen/Qwen3-VL-2B-Instruct",
        "fall": "Qwen/Qwen2-VL-2B-Instruct",
    }

    # Keys consumed from core_allocation.yaml's vlm.<category> entry and forwarded
    # verbatim as from_pretrained kwargs so the mblt-model-zoo config setters
    # (vision_target_cores / text_target_cores / vision_core_mode / text_core_mode
    # plus vision_dev_no / text_dev_no) pin each VLM submodule to its own NPU
    # cores on the intended card. Omitting a *_dev_no key keeps mblt-model-zoo's
    # default (device 0), which matches single-card setups.
    _NPU_CORE_KWARGS = (
        "vision_dev_no",
        "text_dev_no",
        "vision_target_cores",
    ...
    def __init__(self, category: str, core_config: Optional[Dict] = None):
        self._configure_logging()
        self.category = category
        self.core_config = dict(core_config) if core_config else {}
        self.model_lock = RLock()
        self.available_model_ids = self._load_available_model_ids()
        category_default = self.CATEGORY_DEFAULT_MODEL.get(self.category)
        if category_default and category_default in self.available_model_ids:
            self.original_model_id = category_default
        else:
            if category_default is None:
                logging.info(
                    "No CATEGORY_DEFAULT_MODEL entry for category %s; falling back to %s",
                    self.category,
                    self.available_model_ids[0],
                )
            else:
                logging.info(
                    "CATEGORY_DEFAULT_MODEL[%s]=%s is not in available models %s; falling back to %s",
                    self.category,
                    category_default,
    ...
    def _select_device_and_model(self, model_name: str) -> str:
        ...
        if npu_available and not model_name.startswith("mobilint/"):
    ...
    def switch_model(self, requested_model_id: str):
        ...
        with self.model_lock:
            ...
            previous_is_npu = self.is_npu
            logging.info(
                "Switching VLM model from %s (%s) to %s (%s)",
                previous_original_model_id,
                previous_runtime_model_id,
                requested_model_id,
                next_runtime_model_id,
            )
            ...
            try:
                ...
                logging.exception(
                    "Failed to load requested VLM model %s (%s). Reloading previous model %s (%s).",
                    requested_model_id,
                    next_runtime_model_id,
                    previous_original_model_id,
                    previous_runtime_model_id,
                ...
                self.is_npu = previous_is_npu

                try:
                    recovered_model, recovered_processor = self._load_model_and_processor(
                        previous_runtime_model_id
                    )
                    ...
                    self.processor = None
                    logging.exception(
                        "Failed to recover previous VLM model %s (%s).",
                        previous_original_model_id,
                        previous_runtime_model_id,
                    )
                    raise RuntimeError(
    ...
    def _load_model_and_processor(self, model_id: str):
        ...
        if self.is_npu:
            for key in self._NPU_CORE_KWARGS:
                if key in self.core_config:
                    model_kwargs[key] = self.core_config[key]
            logging.info(
                "[%s] Loading NPU model %s with core pinning: %s",
                self.category,
                model_id,
                {k: model_kwargs[k] for k in self._NPU_CORE_KWARGS if k in model_kwargs},
            )
        else:
        ...
        def task():
            try:
                ...
                inputs = processor.apply_chat_template(
                    ...
                    return_dict=True,
                    return_tensors="pt",
                ).to("cpu" if is_npu else "cuda:0")

                # generation_config.<model-name>.json overrides the shared
                # config for models that need different sampling (aya's
                # fluency collapses under the strong anti-repetition
                # penalties the Qwen models require).
                ...
                # active model's own ids so non-Qwen models (e.g. aya)
                # still emit their EOS and stop instead of running to the cap.
                ...
                with get_image_features_callback(model, on_image_processing_done):
                    outputs = model.generate(generation_config=generation_config, **generation_kwargs)
