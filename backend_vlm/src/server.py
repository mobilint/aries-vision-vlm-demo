...
# MLA100 exposes 1 NPU per PCB; MLA400 exposes 4. mbltml only reports the chip
# codename (Aries), so the product name and NPUs-per-card ratio are supplied
# via env so the same code path serves both the dev rig (MLA400) and the demo
# rig (MLA100 x 2).
NPU_PRODUCT_LABEL = os.environ.get("NPU_PRODUCT_LABEL", "MLA400")
try:
    NPU_PER_CARD = max(1, int(os.environ.get("NPU_PER_CARD", "4")))
except ValueError:
    NPU_PER_CARD = 4
...
def init_system_tracker():
    ...
    if cpu_tracker is None:
        try:
            cpu_tracker = CPUDeviceTracker(interval=0.5)
    ...
    if dram_tracker is None:
        try:
            dram_tracker = DRAMDeviceTracker(interval=0.5)
    ...
    if npu_tracker is None:
        try:
            npu_tracker = NPUDeviceTracker(interval=0.5)
...
def get_npu_card_count():
    device_count = get_npu_device_count()
    if not device_count:
        return None
    return max(1, device_count // NPU_PER_CARD)


def get_npu_name():
    card_count = get_npu_card_count()
    if card_count:
        return f"{NPU_PRODUCT_LABEL} x {card_count}" if card_count > 1 else NPU_PRODUCT_LABEL
    return NPU_PRODUCT_LABEL
...
def get_npu_metrics_snapshot():
    ...
    if npu_tracker is None:
        snapshot = {
            "name": get_npu_name(),
            "product_label": NPU_PRODUCT_LABEL,
...
def get_system_metrics_payload():
    ...
    # mblt-tracker's get_metric() returns avg/p99/max over every sample
    # accumulated since the last reset, so without reset() the utilization
    # / temperature / power averages flatten toward the tracker's lifetime
    # baseline (often ~0 % CPU right after startup) and never reflect the
    # most recent second of load. Reset after every read so each poll
    # surfaces a fresh rolling window sized to the frontend's poll cadence.
    with system_metrics_lock:
        snapshot = get_system_metrics_snapshot()
        for tracker in (cpu_tracker, npu_tracker, dram_tracker):
            if tracker is None:
                continue
            try:
                tracker.reset()
            except Exception as exc:
                logging.warning("[system-metrics] tracker reset failed: %s", exc)
